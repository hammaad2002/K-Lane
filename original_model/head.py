...
class RowSharNotReducRef(nn.Module):
    def __init__(self,
        ...
        self.cls_lane_color = cls_lane_color
        self.num_cls = 6
        ...
        for idx_cls in range(self.num_cls):
            setattr(self, f'emb_{idx_cls}', nn.Parameter(torch.randn(dim_token)))
    ...
    def forward(self, x):
        ...
        zero_left = torch.zeros((b_size,dim_feat,img_h,off_grid))
        zero_right = torch.zeros((b_size,dim_feat,img_h,off_grid))
        ...
        for idx_b in range(b_size):
            ...
            for idx_c in range(num_cls):
                ...
                if lane_ext_prob > self.thr_ext: # if the line exts
                    ...
                    temp_token = torch.zeros((dim_feat,img_h,1+2*off_grid))
    ...
    def get_lane_exist_and_cls_wise_maps(self, label_tensor, is_one_hot=False, is_ret_list=True):
        ...
        if is_ret_list:
            ...
            for idx_cls in range(self.num_cls):
                list_ext.append(torch.tensor(np.squeeze(ret_exist[:,idx_cls,:,:])))
                list_cls.append(torch.tensor(np.squeeze(ret_maps[:,idx_cls,:,:])))
    ...
    def get_lane_map_numpy_with_label(self, output, data, is_flip=True, is_img=False, is_get_1_stage_result=True):
        ...
        *       'rgb_img_conf_cls_idx': (144, 144, 3)
        ...
        for batch_idx in range(batch_size):
            ...
            conf_pred = np.where(conf_pred_raw > self.conf_thr, 1, 0)
    ...
    def get_rgb_img_from_cls_map(self, cls_map):
        ...
        for j in range(144):
            for i in range(144):
                ...
                temp_rgb_img[j,i,:] = self.cls_lane_color[idx_lane] \
